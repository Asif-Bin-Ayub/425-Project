program Preject_Part1
    ...
    double precision :: theta,der_theta,omega,der_omega,K,AV,AA,theta0
    character (len=10) length, ini_angle
    ...
    !Path1 (Parabola):---------------------
        !Trajectory:
        write (length,'(f3.0)') l
        write (ini_angle,'(f3.1)') theta0
        open(unit=10,file='P1L('//trim(length)//')T('//trim(ini_angle)//').txt')
        !Equilibrium Point(s):
