...
double precision :: q=0.3d0,Fd=0d0,omega_d=0d0
...
implicit none 
	...
	double precision :: theta_0(1:2), omega_0(1:2), T_0(1:2),theta0
	logical :: check(1:2)
	integer :: c
	...
	!Path1 (Parabola):---------------------
		...
		!Period vs l
			! open(unit=20,file='T_vs_theta0_Path1.txt')
	!Path2 (Cycloid):----------------------
		!Trajectory:
		...
		!Period vs l
			! open(unit=21,file='T_vs_theta0_Path2.txt')

	! loop over theta0 to check the period for different values of theta0
	theta0 = theta0*pi/180d0 !rad
	! do while (theta0<pi)

	!Initializing:=========================
	t=0.0d0 !s
	theta(1:2)=theta0 !rad
	...
	do while(t.le.tmax)
		...
		! Equilibrium Point(s)
		! if (abs(der_omega(1)) .le. 5d-3) then
		...
		Period: if(check(1) .eqv. .FALSE. .and. t>0d0) then
			!Path 1
			if(theta(1).gt. 0d0 .and. abs(omega_0(1)-omega(1)).le. 3d-3) then
				T_0(1)=t
				check(1) = .TRUE.
			end if
		else if(check(2) .eqv. .FALSE.) then
			!Path 2
			if(theta(2) .gt. 0d0 .and. abs(omega_0(2)-omega(2)).le. 3d-3) then
				T_0(2)=t
				check(2) = .TRUE.
			end if		
		end if Period
	...
	! write (21,*) theta0,T_0(2)
	! write (20,*) theta0,T_0(1)

	! theta0=theta0+0.01d0
	! end do
